...
parser.add_argument('--model', default='pointnet2_cls_ssg_spec_cp',help='Model name [default:pointnet2_cls_ssg_spec_cp]')
...
LOG_DIR = 'F:\MLP\代码\classification\log\log'
...
DATA_PATH = r'F:\MLP\mat\数据\modelnet10_normal_resampled'
...
def train():
    with tf.Graph().as_default():
        ...
        for epoch in range(MAX_EPOCH):
            ...
            # np.save(file=r'C:\Users\Administrator\Desktop\mat\数据\保存特征\%d11.npy' % (epoch), arr=l1_xyz)
            # np.save(file=r'C:\Users\Administrator\Desktop\mat\数据\保存特征\%d12.npy' % (epoch), arr=l1_points)
            if epoch == 9:
                # for i in range(3):
                for k in range(BATCH_SIZE):
                    paras = {}
                    paras['mlp'] = l1_points[k:k + 1, :, :].reshape([1024,128])
                    paras['mlp2'] = l1_xyz[k:k + 1, :, :].reshape([1024, 3])
                    # paras['mlp'] = tf.squeeze(l1_points[k:k + 1, :, :])
                    if not os.path.isdir('./保存t/'):
                        os.mkdir('./保存t/')
                    io.savemat('./保存t/' + '%.3d.mat' % k, paras)
