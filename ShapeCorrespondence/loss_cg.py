...
def pointwise_corr_layer(C_est, model_evecs, part_evecs, part_evecs_trans, model_dist_map, part_dist_map, part_vert, model_vert, part_vals, model_vals):
    ...
    #加个重构损失
    part_ms_ = tf.matmul(tf.matmul(part_evecs, part_vals), tf.transpose(part_evecs, [0, 2, 1]))
    model_ms_ = tf.matmul(tf.matmul(model_evecs, model_vals), tf.transpose(model_evecs, [0, 2, 1]))

    part_loss = abs(part_dist_map-part_ms_)
    part_loss_ = tf.reduce_sum(part_loss)
    model_loss = abs(model_dist_map - model_ms_)
    model_loss_ = tf.reduce_sum(model_loss)

    cg_loss = (part_loss_ + model_loss_) / tf.to_float((tf.shape(part_dist_map)[1] * tf.shape(part_dist_map)[1])*2)
    ...
    unsupervised_loss = (unsupervised_loss + cg_loss) / 2.0
